    ...
    context("배너 조회 테스트") {
        ...
        test("날짜와 타입이 매칭된 배너만 나오는지 확인한다.") {
            ...
            val testBanners = listOf(
                // 조건 일치
                testBanner(testWebtoon, HOME_MAIN, baseDateTime.minusDays(1), baseDateTime.plusDays(1)),
                // 조건 일치
                testBanner(testWebtoon, HOME_MAIN, baseDateTime.minusDays(2), baseDateTime.plusDays(2)),
                // 날짜 범위 포함안됨
                testBanner(testWebtoon, HOME_MAIN, baseDateTime.plusDays(2), baseDateTime.plusDays(4)),
                // 날짜 범위 포함안됨
                testBanner(testWebtoon, HOME_MAIN, baseDateTime.minusDays(4), baseDateTime.minusDays(2)),
                // 배너 유형 다름
                testBanner(testWebtoon, NONE, baseDateTime.minusDays(1), baseDateTime.plusDays(1)),
            ).saveAll()
        ...
        test("일치하는 경우 priority asc / id desc 으로 정렬되는지 확인한다.") {
            ...
            val testBanners = listOf(
                // 4번째
                testBanner(testWebtoon, priority = 3),
                // 2번째
                testBanner(testWebtoon, priority = 2),
                // 3번째
                testBanner(testWebtoon, priority = 3),
                // 1번째
                testBanner(testWebtoon, priority = 1),
            ).saveAll()
